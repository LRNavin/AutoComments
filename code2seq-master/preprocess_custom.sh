...
#VAL_DIR=my_val_dir
MAIN_DIR="java_code_data"
DATASET_NAME="sample_set"
...
MAIN_DATA_FILE=${DATASET_NAME}.test.raw.txt
EXTRACTOR_JAR=JavaExtractor/JPredict/target/JavaExtractor-0.0.1-SNAPSHOT.jar

mkdir -p data
mkdir -p data/${DATASET_NAME}

echo "Extracting paths from set..."
${PYTHON} JavaExtractor/extract.py --dir ${MAIN_DIR} --max_path_length 8 --max_path_width 2 --num_threads ${NUM_THREADS} --jar ${EXTRACTOR_JAR} > ${MAIN_DATA_FILE} 2>> error_log.txt
echo "Finished extracting paths from set"
...
rm ${MAIN_DATA_FILE}  ${TARGET_HISTOGRAM_FILE} ${SOURCE_SUBTOKEN_HISTOGRAM} ${NODE_HISTOGRAM_FILE}
