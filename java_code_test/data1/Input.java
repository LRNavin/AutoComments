public void checkNextPoissonConsistency(double VAR0){
  ...
  for (int i=0; i < sampleSize; i++) {
    long value=randomData.nextPoisson(VAR0);
    if (value > maxObservedValue) {
      maxObservedValue=value;
    }
    frequency.addValue(value);
  }
  PoissonDistribution poissonDistribution=new PoissonDistribution(VAR0);
  int lower=1;
  while (poissonDistribution.cumulativeProbability(lower - 1) * sampleSize < minExpectedCount) {
    lower++;
  }
  int upper=(int)(5 * VAR0);
  ...
  if (UnitTestUtils.chiSquareTest(expected,observed) < alpha) {
    ...
    msgBuffer.append("Chisquare test failed for VAR0 = ");
    msgBuffer.append(VAR0);
